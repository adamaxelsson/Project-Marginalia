...
idxs = np.where(labels == 'marginalia')[0]
#print(idxs)
# use the indexes to extract all bounding boxes and associated class
# label probabilities associated with the "marginalia" class
boxes = boxes[idxs]
proba = proba[idxs][:, 1]
...
for (box, prob) in zip(boxes, proba):
	...
	"""
	text= "Marginalia: {:.2f}%".format(prob * 100)
	cv2.putText(clone, text, (startX, y),
		cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 255), 2)
	"""
# show the output after *before* running NMS
#cv2.imshow("Before NMS", clone
cv2.imwrite("/cephyr/users/adamax/Alvis/Project-Marginalia/R-CNN/output/test_images_results/test3.png", clone)
"""
...
for i in boxIdxs:
	...
	cv2.rectangle(image, (startX, startY), (endX, endY),
		(0, 255, 0), 2)
	y = startY - 10 if startY - 10 > 10 else startY + 10
	text= "Marginalia: {:.2f}%".format(proba[i] * 100)
	cv2.putText(image, text, (startX, y),
		cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 2)

cv2.imwrite("/cephyr/users/adamax/Alvis/Project-Marginalia/R-CNN/output/test_images_results/test4.png", image)
