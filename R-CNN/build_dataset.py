...
for (i, imagePath) in enumerate(imagePaths):
	...
	gtBoxes = []
	rows = open(config.ORIG_ANNOTS).read().strip().split("\n")
	# extract the image dimensions
	w = 1000#int(soup.find("width").string)
	h = 700#int(soup.find("height").string)


	# loop over all 'object' elements
	for row in rows[1:]:
		row = row.split(",")
		(imagefile, startX, endX, startY, endY) = row
	...
	for proposedRect in proposedRects[:config.MAX_PROPOSALS]:
		...
		# loop over the ground-truth bounding boxe
		for gtBox in gtBoxes:
			...
			# count limit
			if not fullOverlap and iou < 0.01 and \
