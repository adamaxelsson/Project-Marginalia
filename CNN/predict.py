...
for imagePath in imagePaths:
	...
	image = load_img(imagePath)#, target_size=(350, 500))
	image = img_to_array(image) / 255.0
	image = np.expand_dims(image, axis=0)

	# make bounding box predictions on the input image
	preds = model.predict(image)[0]
	(startX, endX, startY, endY) = preds
	...
	#print(imagePath[-10:])
	cv2.imwrite("/cephyr/users/adamax/Alvis/Project-Marginalia/CNN/output/test_images_results/" + imagePath[-10:], image)
